...
class DatabaseInterface
{
    ...
    public function getUserByONID($onid)
    {
        $query = "
            ...
            SELECT id, onid, email, last_name, first_name, timezone
            FROM meb_user
            WHERE onid = ?
            LIMIT 1
    ...
    public function getDatesByMeetingId($id, $timezone)
    {
        ...
        SELECT start_time AS date FROM meb_timeslot
        WHERE fk_event_id = ?
        ORDER BY date ASC
        ...
        foreach ($list as $start_time) {
            ...
            $timezone_aware_list[ $start_time->format('Y-m-d') ] = true;
        }
        // Convert back to format generated by MySQL query
        $final_result = [];
        foreach (array_keys($timezone_aware_list) as $date) {
    ...
    public function getAvailableDates($hash)
    {
        ...
        SELECT meb_timeslot.start_time AS date
        FROM meb_timeslot
        INNER JOIN meb_event ON meb_event.id = meb_timeslot.fk_event_id
        WHERE meb_event.hash = ?
        ...
        // NOTE: This could be handled by MariaDB itself (with slight modification from
        // commit 14b4089) if its timezone table is given; not currently the case
     ...
     * @return array
     */
    public function getAvailableTimeslots($hash)
    {
        $timeslots_query = "
